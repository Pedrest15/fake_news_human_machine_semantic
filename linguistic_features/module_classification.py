...
Enhanced-UD, depois so com Sintaxe, etc. O objetivo e medir o poder
...
agrupamento de varios grupos (ex.: sintaxe = syllables + pos_tagger +
...
Uso:
    ...
    python module_classification.py --modules nilcmetrics,sintaxe
...
# Modulos de features linguisticas: nome -> grupos de features que o compoem.
# Cada grupo deve ser um nome aceito por LinguisticFeaturesPipeline.feature_groups.
#
# Cada grupo individual e um modulo proprio. Alem disso, 'sintaxe' e um modulo
# agregado que junta os 4 grupos sintaticos (syllables + pos_tagger +
# parser_stats + sage_terms) num unico vetor para uma classificacao so.
MODULES: Dict[str, List[str]] = {
    ...
    'sintaxe': ['syllables', 'pos_tagger', 'parser_stats', 'sage_terms'],
}
...
def run_module(
    ...
    logger.info("\n" + "#" * 70)
    logger.info(f"# MODULO: {module}  (grupos: {', '.join(feature_groups)})")
    logger.info("#" * 70)
    ...
    for clf in classifiers:
        try:
            ...
            logger.error(f"  Modulo {module} / classifier {clf} falhou: {exc}")
...
def print_module_comparison(all_results: Dict[str, Dict[str, Dict]]):
    ...
    logger.info("\n" + "=" * 100)
    logger.info("COMPARACAO POR MODULO (Test F1)")
    logger.info("=" * 100)
    header = f"{'Modulo':<16}" + "".join(f"{c:<16}" for c in classifiers)
    logger.info(header)
    logger.info("-" * 100)
...
def main():
    p = argparse.ArgumentParser(
        description='Classificacao Humano vs LLM por modulo de features linguisticas',
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument('--modules', default='all',
                   help=('Modulos a avaliar separados por virgula '
                         f'({",".join(MODULES)}) ou all'))
    ...
    p.add_argument('--top-k', type=int, default=None,
                   help='SelectKBest sobre o vetor do modulo')
    ...
    if args.modules == 'all':
        ...
        if invalid:
            p.error(f"Modulos invalidos: {invalid}. Validos: {list(MODULES)}")
    ...
    # ---- Comparacao modulo x classificador ----
    comparison = {
        module: {
            clf: {
    ...
    logger.info(f"\nComparacao por modulo salva em: {comp_file}")

    # ---- Melhor classificador por modulo (por Test F1) ----
    best_per_module = {}
    for module, clf_res in all_results.items():
        scored = [
    ...
    logger.info(f"Melhor classificador por modulo salvo em: {best_file}")

    print_module_comparison(all_results)

    logger.info("\nMelhor classificador por modulo (Test F1):")
