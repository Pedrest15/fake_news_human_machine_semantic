...
class LiwcAnalyzer:
    ...
    def load_texts(self):
        ...
        for dir_path in self.human_dirs:
            ...
            print(f"  HUMAN: {dir_path} -> {len(docs)} documents")
        ...
        for dir_path in self.llm_dirs:
            ...
            print(f"  LLM:   {dir_path} -> {len(docs)} documents")
    ...
    def compute_liwc_scores(self):
        ...
        """
        print(f"\n{'='*80}")
        print("COMPUTING PER-DOCUMENT LIWC SCORES")
        print("=" * 80)
        ...
        print(f"\n  Documents processed: {len(self.human_scores) + len(self.llm_scores)}")
        print(f"  LIWC categories: {len(self.category_names)}")
    ...
    def _compute_scores_for_docs(self, docs: list[dict], label: str) -> list[dict]:
        ...
        for i, doc in enumerate(docs):
            ...
            if (i + 1) % 2000 == 0:
                print(f"    {label}: {i + 1}/{len(docs)} documents processed...")

        print(f"  {label.upper()}: {len(scores)} documents scored")
        return scores
    ...
    def analyze_discriminative_categories(self):
        ...
        """
        print(f"\n{'='*80}")
        print("DISCRIMINATIVE ANALYSIS PER LIWC CATEGORY")
        print("=" * 80)

        # Build the per-category arrays
        ...
        print(f"\nCategories analysed: {total}")
        print(f"  Significant (p<0.05): {sig_005} ({100*sig_005/total:.1f}%)")
        print(f"  Significant (p<0.01): {sig_001} ({100*sig_001/total:.1f}%)")
        print(f"  More characteristic of HUMAN: {(self.discriminative_df['characteristic_of'] == 'human').sum()}")
        print(f"  More characteristic of LLM: {(self.discriminative_df['characteristic_of'] == 'llm').sum()}")
    ...
    def export_results(self, output_dir: str):
        ...
        print(f"\n{'='*80}")
        print("EXPORTING LIWC RESULTS")
        print("=" * 80)
        print(f"Directory: {output_dir}")

        # 1. Discriminative categories (CSV)
    ...
    def _write_report(self, output_file: Path):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("=" * 110 + "\n")
            f.write("LIWC DISCRIMINATIVE ANALYSIS: HUMAN vs LLM\n")
            f.write("=" * 110 + "\n\n")

            f.write("HOW TO READ THIS REPORT:\n")
            f.write("-" * 110 + "\n")
            f.write("  - mean_human/mean_llm: mean % of words in the category for each group\n")
            f.write("  - difference: (mean_human - mean_llm)\n")
            f.write("      > 0: category more frequent in HUMAN texts\n")
            f.write("      < 0: category more frequent in LLM texts\n")
            ...
            f.write("-" * 110 + "\n")
            f.write(f"  Human documents: {len(self.human_scores)}\n")
            f.write(f"  LLM documents: {len(self.llm_scores)}\n")
            f.write(f"  Total: {len(self.human_scores) + len(self.llm_scores)}\n")
            f.write(f"  LIWC categories: {len(self.category_names)}\n\n")

            sig_005 = df['significant_005'].sum()
            sig_001 = df['significant_001'].sum()
            f.write("ANALYSIS SUMMARY:\n")
            f.write("-" * 110 + "\n")
            f.write(f"  Significant categories (p<0.05): {sig_005}\n")
            f.write(f"  Significant categories (p<0.01): {sig_001}\n")
            ...
            f.write("=" * 110 + "\n")
            f.write("ALL LIWC CATEGORIES (sorted by absolute difference)\n")
            f.write("=" * 110 + "\n\n")

            f.write(f"{'#':<4}{'Category':<40}{'Mean_H':>8}{'Mean_L':>8}{'Diff':>9}{'p-value':>11}{'Cohen d':>9}{'Charact.':>10}\n")
            f.write("-" * 110 + "\n")
    ...
    def _write_summary(self, output_file: Path):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("LIWC DICTIONARY:\n")
            f.write(f"  Categories: {len(self.category_names)}\n")
            f.write(f"  Exact words: {len(self.dictionary.exact_matches)}\n")
            f.write(f"  Prefixes: {len(self.dictionary.prefix_matches)}\n\n")

            # Mean dictionary coverage
            ...
            f.write(f"  Human mean: {human_cov:.1f}% of words recognised\n")
            f.write(f"  LLM mean:   {llm_cov:.1f}% of words recognised\n\n")

            # Mean word count
            human_wc = np.mean([s['word_count'] for s in self.human_scores])
            llm_wc = np.mean([s['word_count'] for s in self.llm_scores])
            f.write("WORD COUNT:\n")
            f.write(f"  Human mean: {human_wc:.1f} words/document\n")
            f.write(f"  LLM mean:   {llm_wc:.1f} words/document\n\n")

            if self.discriminative_df is not None:
                df = self.discriminative_df
                sig = df[df['significant_005']]

                f.write("SIGNIFICANT CATEGORIES (p<0.05):\n")
                f.write("-" * 80 + "\n")
